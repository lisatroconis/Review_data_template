...
file <- "C:/Users/lisa.reiners/OneDrive - ACTED/Documents/REG2302/Panama/inputs/PANAMA_Research_cycle_data_cleaning_HQ.xlsx"

raww <- read_excel(file, guess = 10000, sheet = "Raw_Data")
...
logg <- read_excel(file, guess = 50000,sheet = "Logbook")
#colnames(logg) <- logg[1,]
#logg <- logg[-1, ]


dell <- read_excel(file, guess = 50000, sheet = "Deletion log")

questions <- read_excel(file, sheet="KOBO_survey")

choices <- read_excel(file, sheet = "KOBO_choices")
...
# Sort the dataframe based on the 'blanks' column in descending order
cleann <- cleann[order(cleann$blanks, decreasing = TRUE), ]

# Create a vector of colors based on the condition (blanks > 50%)
blank_colors <- ifelse(cleann$blanks > 0.5, "FF0000", "FFFFFF")  # Red color for values > 50%, white otherwise
