...
process_datasets_and_extract_uuids <- function(datasetList) {
  for (name_datasets in names(datasetList)) {
    dataset <- datasetList[[name_datasets]]  # Get the current dataset
    
    have_uuid <- FALSE
    column_names <- colnames(dataset)
    duplicate_columns <- duplicated(column_names)
    
    if (any(duplicate_columns)) {
      duplicate_column_names <- column_names[duplicate_columns]
    ...
    if (!have_uuid) {
      ...
      if (column_found) {
        assign(paste("uuid_", name_datasets, sep = ""), uuid_name_datasets, envir = .GlobalEnv)
      } else {
...
# Call the function
uuid_names <- process_datasets_and_extract_uuids(datasetList)

uuid_raww <- uuid_names$uuid_raww
uuid_cleann <- uuid_names$uuid_cleann
uuid_logg <- uuid_names$uuid_logg
uuid_dell <- uuid_names$uuid_dell


#names from the cleaning log
extract_logg_names <- function(logg) {
  ...
  return(list(
    new_value_logg = new_value_logg,
    old_value_logg = old_value_logg,
    var_logg = var_logg
  ))
}

# Access the individual variables
logg_names <- extract_logg_names(logg)
new_value_logg <- logg_names$new_value_logg
old_value_logg <- logg_names$old_value_logg
var_logg <- logg_names$var_logg
